...
def generate_boxplot(df, story):
    ...
    filename = f"backend/charts/box_{num_col}_by_{cat_col}.png"
...
def generate_scatter(df, story):
    ...
    filename = f"backend/charts/scatter_{x}_{y}.png"
    plt.title(f"{x} vs {y}")
    plt.savefig(filename)
    plt.close()
    return {
...
def generate_histogram(df, story):
    ...
    filename = f"backend/charts/hist_{col}.png"
...
def generate_category_bar(df, story):
    ...
    filename = f"backend/charts/bar_{col}.png"
...
def generate_grouped_bar(df, story):
    ...
    if num_col not in df.columns or cat_col not in df.columns or not pd.api.types.is_numeric_dtype(df[num_col]):
        ...
        plt.xticks(rotation=45, ha="right")
        filename = f"backend/charts/bar_story_{num_col or 'metric'}_by_{cat_col or 'group'}.png"
    ...
    plt.xticks(rotation=45, ha="right")
    filename = f"backend/charts/bar_{num_col}_by_{cat_col}.png"
...
def generate_heatmap(df, story):
    ...
    filename = f"backend/charts/heatmap_{'_'.join(story.get('columns', ['categorical']))}.png"
...
def generate_regression_plot(df, story):
    ...
    sns.scatterplot(data=df, x=x, y=y)
    sns.regplot(data=df, x=x, y=y, scatter=False, color="red")
    filename = f"backend/charts/regression_{x}_{y}.png"
...
def generate_predictive_driver_bar(df, story):
    ...
    filename = f"backend/charts/predictive_drivers_{story.get('column', 'target')}.png"
...
def generate_prediction_fit_plot(df, story):
    ...
    filename = f"backend/charts/prediction_fit_{story.get('column', 'target')}.png"
